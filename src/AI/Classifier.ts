...
export default class Classifier {
  ...
  createExample(results) {
    const example = tf.tensor(
      results.poseLandmarks.reduce(function (array, data_point) {
    ...
    example.concat(
      tf.tensor(
        results.leftHandLandmarks.reduce(function (array, data_point) {
          array.push(data_point.x);
          array.push(data_point.y);
          array.push(data_point.visibility);
          return array;
    ...
    example.concat(
      tf.tensor(
        results.rightHandLandmarks.reduce(function (array, data_point) {
          array.push(data_point.x);
          array.push(data_point.y);
          array.push(data_point.visibility);
          return array;
  ...
  call(keypoints) {
    if (
      ...
      keypoints.poseLandmarks.length > 0 &&
      keypoints.leftHandLandmarks &&
      keypoints.leftHandLandmarks.length > 0 &&
      keypoints.rightHandLandmarks &&
      keypoints.rightHandLandmarks.length > 0
    ) {
      if (this.isLearning) {
